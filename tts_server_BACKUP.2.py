...
# Markdown stripping to plain text, e.g. insert newlines after block elements so that lists become lines
def _md_plain_element(element, stream=None):
    if stream is None:
        stream = StringIO()
    if element.text:
        stream.write(element.text)
    for child in element:
        _md_plain_element(child, stream)
        if getattr(child, "tag", "").lower() == "br":
            stream.write("\n")
    if getattr(element, "tag", "").lower() in {
        "p","li","ul","ol","div","section","article",
...
Markdown.output_formats["plain"] = _md_plain_element
...
# Inline enumerators like " … 2. This … " → newline (to split inline lists)
_INLINE_ENUM = re.compile(r"(?<!^)(?<!\n)\s*(?:\d{1,3}[.)]|[a-z]\))\s+(?=[A-Z])")

def _normalize_text_for_tts(text: str) -> str:
    """
    - Markdown → plain (adds newlines after list items/paragraphs)
    - Strip emojis
    - Split inline enumerators into newlines
...
@Language.factory("list_sentencizer_factory")
def create_list_sentencizer(nlp, name):
    return list_sentencizer

def list_sentencizer(doc):
    """
    Handles sentence splitting for numbered lists.
    """
    for i, token in enumerate(doc[:-1]):
        # Check if the token is a digit followed by a period
        if token.is_digit and doc[i+1].text == ".":
            # This is a list item, so prevent a sentence break
            doc[i+1].is_sent_end = False
    return doc
...
def _get_sentencizer(lang_code: str):
    ...
    # Remove the default sentencizer if it exists
    if "sentencizer" in nlp.pipe_names:
        nlp.remove_pipe("sentencizer")

    # If a parser is present, remove it to prevent it from setting sentence boundaries
    if "parser" in nlp.pipe_names:
        nlp.remove_pipe("parser")
        
    # Now, add our custom component
    if "list_sentencizer_factory" not in nlp.pipe_names:
        nlp.add_pipe("list_sentencizer_factory", first=True)
...
def _get_sentencizer(lang_code: str):
    ...
    # NEW: Add your custom component to the pipeline.
    if "list_sentencizer" not in nlp.pipe_names:
        nlp.add_pipe("list_sentencizer", first=True)

    if "senter" in nlp.pipe_names:
        pass
    elif "sentencizer" not in nlp.pipe_names:
        # We need to make sure our list sentencizer runs BEFORE the default.
        # Spacy's sentencizer automatically looks for periods and splits.
        # Our custom component explicitly tells it not to.
...
async def tts_text_chunk(sid, data):
    ...
    try:
        ...
        # Idle-timeout flush: only flush at true boundaries (punctuation or newline)
        now = time.time()
        ...
        if (not is_final) and (not to_speak) and keep_tail and (now - last) >= idle_timeout_s:
            if re.search(r"[.!?…。\n\r]$", keep_tail):
                ...
                keep_tail = ""
            # else: hold buffer (likely mid-phrase)
        ...
        for sentence in to_speak:
            ...
            # Clean via Markdown + Emoji (single, centralized step)
            s_clean = _normalize_text_for_tts(s_raw)

            # Detokenize (EN/PT)
            try:
                s_detok = _detok_sentence(s_clean, norm_lang)
            except Exception as detok_err:
                logger.warning(f"Detokenize failed for lang={norm_lang}: {detok_err}")
                s_detok = s_clean

            # Ensure a tiny pause at the end
            if not re.search(r"[.!?…。？！]$", s_detok):
                s_detok += "."

            logger.info(f"TTS: {s_detok[:60]}... (voice: {voice}, lang: {norm_lang}, speed: {speed})")
            ...
            for audio_sid in audio_sids:
                try:
                    ...
                    if format_type == "binary":
                        async for metadata, binary_data in generate_tts_binary(s_detok, voice, speed, target_client_id):
                        ...
                        async for chunk_data in generate_tts_base64(s_detok, voice, speed, target_client_id):
