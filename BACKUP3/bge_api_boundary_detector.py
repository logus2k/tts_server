...
class APIBasedBGEBoundaryDetector:
    ...
    Optimized for streaming TTS with caching and batching
    """
...
class APIStreamingSentenceBuffer:
    ...
    Adapted for your existing FlagEmbedding infrastructure
    """
    ...
    def __init__(self, client_id: str, settings: Dict = None):
        ...
        # Timing configuration (slightly more conservative for API calls)
        self.completion_delay = 0.7  # Slightly longer for API latency
        self.force_completion_delay = 2.0
        self.semantic_check_interval = 0.5  # Less frequent API calls
        
        # BGE-M3 API integration
        ...
        self.semantic_confidence_threshold_timed = 0.65
        
        # State tracking
        self.last_apparent_completion = None
        
        logger.info(f"API-based StreamingSentenceBuffer initialized for client {client_id}")
    ...
    async def add_chunk(self, chunk: str) -> List[Tuple[str, Dict]]:
        ...
        # 3. FORCE COMPLETION: Prevent hanging
...
if __name__ == "__main__":
    async def test_api_bge_detector():
        """Test the API-based BGE boundary detector"""
        
        # Load settings
        settings = await load_flagembedding_settings()
        
        # Create buffer
        buffer = APIStreamingSentenceBuffer("test_client", settings)
        
        # Test health check
        health_ok = await buffer.semantic_detector.api_client.check_health()
        print(f"🔍 API Health Check: {'✅ OK' if health_ok else '❌ Failed'}")
        
        if not health_ok:
            print("⚠️ FlagEmbedding API service is not available")
            return
        
        # Sample streaming conversation
        stream_chunks = [
            "Hello there",
            ". How are you",
            " doing today",
        ...
        print("🧪 Testing API-based BGE-M3 Boundary Detection")
        print("=" * 60)
        
        for i, chunk in enumerate(stream_chunks):
            print(f"\nChunk {i+1}: '{chunk}'")
            
            ready_sentences = await buffer.add_chunk(chunk)
            
            for sentence, analysis in ready_sentences:
                print(f"  ✅ COMPLETE: {sentence}")
                print(f"     Method: {analysis['method']}")
                print(f"     Confidence: {analysis['confidence']:.3f}")
                print(f"     API Time: {analysis.get('processing_time_ms', 0):.1f}ms")
                print(f"     API Used: {analysis.get('api_used', False)}")
            
            if not ready_sentences:
                print(f"     Buffer: '{buffer.buffer}' (length: {len(buffer.buffer)})")
        ...
        print(f"\n📊 Performance Stats:")
        print(f"   API Requests: {stats['api_performance']['total_requests']}")
        print(f"   Avg Response: {stats['api_performance']['average_response_time_ms']:.1f}ms")
        print(f"   Cache Hits: {stats['api_performance']['cache_hits']}")
        print(f"   Cache Misses: {stats['api_performance']['cache_misses']}")
    ...
    # Run test
    try:
        asyncio.run(test_api_bge_detector())
    except Exception as e:
